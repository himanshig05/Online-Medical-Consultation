...
const PatientPayments = () => {
  ...
  useEffect(() => {
    if (router.isReady && patientEmail) {
      const fetchHistory = async () => {
        try {
          const response = await axios.get(`${BASE_URL}/payment/view-patient/${patientEmail}`);
      ...
      {history.length > 0 ? (
        history.map((payment, index) => (
          ...
          >
            ...
            >
              <span>Doctor: {payment.doctorEmail}</span>
              <span
                style={{
